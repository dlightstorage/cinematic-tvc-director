...
export async function terminalStudio() {
  if (!stdin.isTTY) throw new Error('tvc studio --terminal needs an interactive terminal.');
  ...
  try {
    while (true) {
      ...
      if (choice === '1') call(['setup']);
        ...
        const provider = (await terminal.question('Provider (blank for overview): ')).trim();
        call(['plans', ...(provider ? [provider] : [])]);
  ...
  return 'Terminal control room closed.';
}
